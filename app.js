...
const authData = JSON.parse(
  fs.readFileSync(path.join(dir_base, "auth.json"), "utf8")
);
...
if (!fs.existsSync(path.join(dir_base, "backups")))
  fs.mkdirSync(path.join(dir_base, "backups"));
...
app.use(
  app_context + "/static",
  express.static(path.join(dir_base, "public/"))
);
app.use(
  app_context + "/font-awesome",
  express.static(path.join(dir_base, "node_modules/font-awesome/"))
);
app.use(
  app_context + "/jquery",
  express.static(path.join(dir_base, "node_modules/jquery/dist/"))
);
app.use(
  app_context + "/bootstrap",
  express.static(path.join(dir_base, "node_modules/bootstrap/dist/"))
);
app.use(
  app_context + "/css",
  express.static(path.join(dir_base, "public/css"))
);
app.use(
  app_context + "/fonts",
  express.static(path.join(dir_base, "public/fonts"))
);
app.use(app_context + "/js", express.static(path.join(dir_base, "public/js")));
app.use(
  app_context + "/favicon.ico",
  express.static(path.join(dir_base, "public/favicon.ico"))
);
  ...
  function(err) {
    ...
    app
      .listen(app_port, function() {
        console.log(
          "adminMongo listening on " + app_host + ":" + app_port + app_context
        );
